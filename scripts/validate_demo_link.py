...
def main() -> None:
    ...
    allowed_statuses = {"live", "pending-pages-deployment", "pending-github-pages-support"}
    if demo_status not in allowed_statuses:
        raise SystemExit(
            "DEMO_STATUS must be one of: " + ", ".join(sorted(allowed_statuses))
        )
    missing = []
    for relative_path in DOCS_REQUIRED:
        ...
        if demo_status not in text and relative_path != "docs/demo/index.html":
            missing.append(f"{relative_path} missing status {demo_status}")
